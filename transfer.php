 ...
 <a class="navbar-brand" href="#">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active ">  <a class="nav-link" href="transfer.php">Transaction</a></li>
...
<div class="container-fluid">
  ...
  <div class="background text-white text-center">
      ...
      <div class="background text-white text-center">
            ...
            <button type="submit" name="get" class="btn btn-dark btn-bloc btn-sm my-1">Get Account Info</button>
          </div>
      </form>
          ...
          { $row2 = $array2->fetch_assoc();
            echo "<div class='alert alert-success w-50 mx-auto'>
                  <form method='POST'>
                    ...
                    <button type='submit' name='transfer' class='btn btn-dark btn-bloc btn-sm my-1'>Tranfer</button>
                  </form>
                </div>";
          }elseif ($array3->num_rows > 0) {
           $row2 = $array3->fetch_assoc();
            echo "<div class='background text-white text-center w-50 mx-auto'>
                  <form method='POST'>
                    Account No.
                    <input type='text' value='$row2[accountNo]' name='otherNo' class='form-control ' readonly required>
                    Account Holder Name.
                    ...
                    Enter Amount for tranfer.
                    <input type='number' name='amount' class='form-control' min='1' max='$userData[balance]' required>
                    <button type='submit' name='transferSelf' class='btn btn-dark btn-bloc btn-sm my-1'>Tranfer</button>
                  </form>
                </div>";
          }
          else
    ...
    if (isset($_POST['transferSelf']))
    {
      ...
      makeTransaction('transfer',$amount,$userData['balance'],$_POST['otherNo']);
    ...
    if (isset($_POST['transfer']))
    {
      ...
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='background text-white text-center list-group-item list-group-item-action '>Transfer have been made for  Rs.$row[debit] from your account at $row[date] in  account no.$row[other]</div>";
            }
