...
<div class="card w-100 text-center shadowBlack">
  <div class="card-header">
    Send Notice to <?php echo $row['c_id'] ?>
  </div>
  <div class="card-body">
    <form method="POST">
          <div class="alert alert-success w-50 mx-auto">
            <h5>Write notice for <?php echo $row['c_id'] ?></h5>
            <input type="hidden" name="userId" value="<?php echo $row['c_id'] ?>">
            <textarea class="form-control" name="notice" required placeholder="Write your message"></textarea>
            <button type="submit" name="send" class="btn btn-dark btn-block btn-sm my-1">Send</button>
          </div>
      </form><?php
    if (isset($_POST['send']))
    {
